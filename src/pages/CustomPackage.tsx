...
const CustomPackage = () => {
  ...
  const renderPrice = (currentPrice, originalPrice = null) => {
    ...
    return (
      <div className="flex flex-col items-start">
        <div className="flex items-baseline gap-2">
          <span className="text-xl md:text-2xl font-bold text-[#0A7DCF]">₹{currentPrice.toLocaleString()}</span>
          <span className="text-sm md:text-base text-gray-500 line-through">₹{originalPrice.toLocaleString()}</span>
        </div>
        <span className="text-sm md:text-base font-medium text-green-600">{discountPercent}% OFF</span>
      </div>
    );
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {labs.map(lab => {
              ...
              return (
                <div
                  key={lab.id}
                  onClick={() => handleLabSelect(lab)}
                  className={`p-6 bg-white rounded-2xl shadow-md border-2 cursor-pointer transition-all hover:shadow-lg ${
                    selectedLab?.id === lab.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-blue-300'
                  }`}
                >
                  <h3 className="text-xl font-bold text-[#0A7DCF] mb-2">{lab.name}</h3>
                  <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
                    <span>{lab.location}</span>
                    <span className="flex items-center gap-1">
                      <Star className="w-4 h-4 text-yellow-400 fill-current" /> {lab.rating}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-sm border-t pt-4">
                    <div>
                      <p className="text-gray-500">Tests Total</p>
                      <p className="font-semibold">₹{testsTotal.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Lab Fee</p>
                      <p className="font-semibold">₹{lab.extraFee.toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex justify-between items-baseline">
                      <span className="text-gray-500">Final Amount</span>
                      <span className="text-xl font-bold text-[#0A7DCF]">₹{finalAmount.toLocaleString()}</span>
                    </div>
                    <p className="text-green-600 text-sm">{discountPercent}% OFF • Save ₹{discountAmount}</p>
                  </div>
                  <div className="text-center mt-6 text-lg font-bold text-green-700">
                    Pay only ₹{payable.toLocaleString()}
                  </div>
                </div>
              );
          ...
          <div className="mt-12 flex flex-col sm:flex-row justify-center gap-4">
            <button
              ...
              disabled={!selectedLab}
              className={`px-10 py-4 rounded-xl font-bold text-white transition ${
                selectedLab ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'
              }`}
            >
              Proceed to Book the Tests
            </button>
